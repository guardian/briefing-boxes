(function() {
  (function($) {
    ...
    addTracksToPlaylist = function(username, playlist, tracks, callback) {
      var url;
      url = 'https://api.spotify.com/v1/users/' + username + '/playlists/' + playlist + '/tracks' + '?uris=' + tracks;
      return $.ajax(url, {
        method: 'POST',
        dataType: 'text',
        headers: {
    ...
    return $.fn.spotify_add_to_playlist = function(options) {
      ...
      this.each(function() {
        ...
        if (location.hostname === "localhost") {
          redirect_uri = 'https://interactive.guim.co.uk/embed/article-embeds/spotify/embed.html';
        } else {
          redirect_uri = 'https://interactive.guim.co.uk/embed/article-embeds/spotify/embed.html';
        }
